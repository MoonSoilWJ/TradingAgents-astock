...
- **代码级退潮闸**：情绪周期=退潮期(晋级率<30% 且 昨日涨停均涨<1%) 或 冰点期(<40% 且 <0%) → 买侧暂停推送，只记判定；周期转好自动恢复（走额度恢复解冻逻辑）
- 卖侧：**T+1 14:30 强制了结**（封板豁免）；封死涨停代码级禁卖；早盘 10:00 前禁卖；SELL_THR=60；**推送即平仓**（推送过卖出提醒 = 记账+移出持仓+释放额度；没真卖用 `--undo-sold CODE` 撤销）
