    ...
    for ev in events:
        if "buy_price" in ev and "sell_price" not in ev:
            ...
            closed.append({
                ...
                "signal_time": ev.get("signal_time") or buy.get("signal_time"),
                "buy_time": ev.get("buy_time") or buy.get("buy_time"),
                "sell_time": ev.get("sell_time"),
...
def closed_to_table_rows(closed: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for c in closed:
        ...
        rows.append({
            ...
            "信号时间": c.get("signal_time"),
            "买入时间": c.get("buy_time"),
            "平仓日": c.get("sell_date"),
            "卖出时间": c.get("sell_time"),
            "代码": c.get("etf"),
            "标的": c.get("name"),
