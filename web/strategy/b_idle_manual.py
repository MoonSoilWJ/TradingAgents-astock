...
def render_b_idle_validation() -> None:
    ...
    with st.expander("📊 Walk-Forward 样本外总览 (去偏差·2026-07-31)"):
        ...
        st.markdown("> **2026-08-12 本地 SHADOW R3 与聚宽 A 版对齐(终态)**: 本地 SHADOW R3 已与聚宽 joinquant_unified_single.py 的 A 版(产出 2014-2026 +1861%/夏普1.01)逐字对齐: ①选股时点 = 14:40 锁领头羊(--pick) + 14:45 复核成交(--signal, 要求 14:45 仍 ≥3%), 不用 14:45选+14:40回溯(那会漏掉尾盘爆发的真正赢家); ②中性 regime 当月月池缺失时回退'全并集'(R3_UNIVERSE, 等价聚宽 ATTACK_UNIVERSE), 不用'最近非空月池'。趋势/震荡→从R3宇宙按近30天动量取Top25滚动优质池(不变)。日K 经 akshare fund_etf_hist_sina 拉取缓存于 r3_daily.json(每天至多一次); 缺失/网络失败降级为中性→全并集。cron: 40 14 --pick / 45 14 --signal / 40 9 --sell-loop。注意: 聚宽8月初震荡静默期(无≥3%标的)的0交易是设计内的正常表现, 非bug。")
...
def render_b_idle_rules() -> None:
    ...
    # ── 6. 配对收敛薄补充腿 ──
    st.markdown("**⑥ 配对收敛薄补充腿 (`t0_pair_shadow.py` · 核心B腿熄火时非趋势期点缀)**")
    st.markdown(
        "- **定位**: idle 隔夜动量腿(无偏数据重验为负期望)已停用, 此腿填补「核心B腿熄火日」的 idle 资金窗口。"
        "仅在 **动量熄火(全市场无 T0 ETF 日内涨≥3%) 且 regime≠趋势(非趋势)** 时部署干净子宇宙配对收敛, 小仓位点缀。\n"
        "- **子宇宙**: GOLD / NASDAQ / HSCEI 双胞胎(剔除不回归的 HSTECH/HKINTERNET), 共 18 对 (含 NASDAQ 513100/159941/513300 三对)。\n"
        "- **信号**: 14:55 取 A/B 收盘, 算 log 比值 z-score; `|z| ≥ 2.5` 且 `|s-μ| ≥ 0.005` 入场(买偏离侧), "
        "隔夜持有至 `|z| ≤ 0.3`(回归) 或 `≤ 3 天` 平。\n"
        "- **成绩(全4年干净数据, 已修 pre2024 ×5 脏数据)**: NASDAQ `66笔 / +50.6% / 均值+0.73%笔 / 胜56%` = 真实薄边缘; "
        "GOLD `+4704%` 属趋势期时代效应(与动量腿重叠, 不计入独立贡献)。复合%含复利失真(单笔极值), 真实每笔期望 ≈ 0.5~0.7%。\n"
        "- **独立隔离**: 独立 `pair_shadow_state.json` + `pair_shadow_journal.jsonl`; **仅记录, 绝不真下单**; "
        "仓位 `PAIR_CAPITAL_PCT=0.15` (小仓位验证真实成交价)。\n"
        "- **安装**: `bash scripts/install_crontab.sh --install-pair-shadow` (15:05 信号 / 14:55 次日平仓)。"
    )
