...
## Week 8 — 板块轮动监控系统（2026-07-11）✅ 已完成

**目标：验证动量公式有效性 + 搭建板块轮动监控系统 + 钉钉推送**

### 公式验证过程

从零开始验证动量公式，经过 6 轮迭代找到最优参数：

| 版本 | 公式 | 领涨股收盘涨率 | ETF 收盘涨率 | 结论 |
|------|------|---------------|------------|------|
| v1 | 3日窗口 加权 4 因子 | 53.8% | — | 弱有效 |
| v2 | 1日窗口 重调权重 | 60.7% | — | 有效 |
| v3 | 10日窗口 + 入场过滤 | 46.2% | — | 过滤反而降质 |
| v4 | 10日 × 量价配合 | 60.0% | 33.3% | 量价有效 |
| v5 | 10日 × 量价 × CMF 资金流 | 63.3% | 37.5% | CMF 对领涨股有效 |
| **v6** | **3日 × 量价(阈1.5 均5 底0.3)** | — | **52.6%** | **ETF 最优** |

**关键发现 — 领涨股验证严重高估公式效果：**
- 领涨股自带动量加成，波动远大于 ETF（平均涨幅 +2.07% vs +0.34%，差 6 倍）
- v5 在领涨股上收盘涨率 63.3%，换 ETF 只有 37.5%
- CMF 资金流因子对 ETF 基本无效（ETF 是一篮子股票，个股资金流规律被稀释）
- 量价因子对 ETF 有效（+5.3pp），但参数值不敏感（阈值 1.5 和 2.0 结果几乎一样）

**深度调参（73 种组合）：**
- 窗口：1/2/3/5/7/10 日 × 4 种组合（纯涨幅/量价/资金流/量价+资金流）
- 固定 3 日窗口后：量比阈值 1.5/2.0/3.0 × 量均周期 3/5/10 × 量能底数 0.3/0.5/0.7
- CMF 周期 3/5/10 × CMF 乘数 0.5/1.0/1.5/2.0 × CMF 底线 0.3/0.5/0.7
- 加法结构 vs 乘法结构对比
...
**验证结果（ETF，57 样本）：**
- 次日收盘上涨率: 52.6%（19/36）
- 次日盘中最高上涨率: 77.2%（28/36）
- 持续性（TOP5 跑赢 BOTTOM5）: 69.2%（19 天）
- 平均次日收盘涨幅: -0.157%
- 平均次日盘中最高涨幅: +1.448%
...
| 文件 | 用途 |
|------|------|
| `scripts/validate_rotation_momentum.py` | 公式验证脚本：领涨股 + ETF 双轨验证，73 种参数深度搜索 |
| `scripts/rotation_monitor.py` | 轮动监控系统：v6 公式，ETF 优先排名，钉钉推送 |
| `scripts/install_crontab.sh` | crontab 安装脚本：每交易日 15:30 自动运行 |

### 监控系统设计

**数据链路：**
1. 新浪板块列表 → 49 个行业板块
2. 板块 → ETF 映射（37 个有 ETF，12 个用领涨股代理）
3. 新浪 K 线 API（30 日日 K）→ 计算 3 日涨幅 + 量能因子
4. v6 得分排名 → TOP5/BOTTOM5
5. 与上次状态对比 → 检测轮动信号（新进/退出 TOP5）
6. 钉钉 markdown 推送 + 状态持久化
...
无 ETF 的 12 个板块（印刷包装、次新股、纺织等）仅展示不参与排名。

**使用方式：**
```bash
# 日常运行（推送钉钉）
python3 scripts/rotation_monitor.py

# 仅打印不推送
python3 scripts/rotation_monitor.py --dry-run

# 仅轮动信号时推送
python3 scripts/rotation_monitor.py --alert-only

# 测试钉钉配置
python3 scripts/rotation_monitor.py --test-push

# 安装定时任务（每交易日 15:30 自动运行）
bash scripts/install_crontab.sh
```

**钉钉配置：**
- `.env` 中 `DINGTALK_WEBHOOK` 填 webhook 地址
- `DINGTALK_KEYWORD` 填安全关键词（默认 `轮动`）
- `DINGTALK_SECRET` 填加签密钥（可选）

**状态文件：** `~/.tradingagents/rotation/monitor_state.json`

### 验证数据归档

验证原始数据保存在 `~/.tradingagents/rotation/validation_*.json`，包含：
- 49 个板块领涨股 K 线 + 日收益率
- 37 个 ETF K 线 + 日收益率
- 全部验证结果（5 种公式 × 持续性/排名稳定性/轮动信号质量/窗口对比）
- 73 种深度调参结果
...
| 问题 | 原因 | 解决 |
|------|------|------|
| 新浪 API 直连走代理返回空 | 代理对新浪 HTTP 接口不稳定 | curl_get 先不走代理，失败再走代理 |
| 东方财富资金流 API 被封 | push2his.eastmoney.com 返回 Empty reply | 用 CMF（Chaikin Money Flow）从 OHLCV 计算资金流代理 |
| 领涨股验证高估公式效果 | 领涨股自带动量加成，不能代表板块 | 改用 ETF K 线验证，领涨股仅做无 ETF 板块的 fallback |
| 领涨股和 ETF 混合排名不公平 | 领涨股波动大，3 日涨幅远高于 ETF | 排名只用 ETF 板块，领涨股板块单独展示 |
| CMF 对 ETF 无效 | ETF 是一篮子股票，个股资金流规律被稀释 | v6 去掉 CMF，只用量价因子 |
...
- ✅ **Week 8 板块轮动监控** — 动量公式验证（6 轮迭代，73 种参数搜索）+ v6 公式定版（3日涨幅 × 量价因子，ETF 收盘涨率 52.6%）+ 监控系统（ETF 优先排名 + 钉钉推送 + crontab 定时）
- ✅ **T+0 双时点确认定版（2026-07-29）** — 买入规则升级为「14:40 与 14:45 涨幅均≥3% 的 TOP1 → 14:50 买入」，防尾盘脉冲踩线单。100 日回测（3-05~7-27，11:05 强平口径）：无确认 75 笔 +92.68%；双时点 14:40 版 71 笔 **+103.15%**——纯增量过滤，只触发 4 次（拦 3 笔亏损 + 误杀 1 笔 +3.01% 盈利）。同时修复回测-实盘口径 bug：此前回测用 `--confirm-time 14:45` 而实盘 `CONFIRM_TIME="14:40"`，已统一为 14:40，`BUY_RULE` 推送文案同步（`scripts/t0_monitor.py`）。
